...
const PeriodicReport: React.FC<Props> = ({ releases, records, t }) => {
  ...
  const reportData = useMemo(() => {
    ...
    const periodRecords = records.filter(r => {
      const d = new Date(normalizeToDateStr(r.date));
      return d >= from && d <= to;
    });
                  ...
                  <tr key={site} className="hover:bg-gray-50">
                    <td className="px-6 py-4 border">
                      <div className="flex flex-col">
                        <span className="font-black text-gray-800">{site}</span>
                        {CUSTOMERS.find(c => c.name === site) && (
                          <span className="text-[10px] font-black text-indigo-600">كود: {CUSTOMERS.find(c => c.name === site)?.code}</span>
                        )}
                      </div>
                    </td>

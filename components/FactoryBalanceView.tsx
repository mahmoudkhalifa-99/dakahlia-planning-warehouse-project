...
const FactoryBalanceView: React.FC<Props> = ({ releases, records, factoryBalances, onNotify, canEdit, lang, selectedMaterial }) => {
  ...
  const data = useMemo(() => {
    ...
    factoryBalances.forEach(f => allSites.add(normalize(f.factoryName || '')));
    ...
    allSites.forEach(siteNorm => {
      ...
      const originalName = releases.find(r => normalize(r.siteName || '') === siteNorm)?.siteName || 
                           ...
                           factoryBalances.find(f => normalize(f.factoryName || '') === siteNorm)?.factoryName || siteNorm;

      const key = `${siteNorm}||${activeMaterialName}`;
      const manual = factoryBalances.find(fb => 
        normalize(fb.factoryName || '') === siteNorm && String(fb.goodsType || '').includes(activeMaterialName)
      );
      ...
      // النقله بتاريخ الأمس تظهر في الوارد
      const yesterdayArrived = records
        .filter(r => {
          const recordDate = normalizeToDateStr(r.date);
          const siteTarget = normalize(r.unloadingSite || r.customerName);
          return siteTarget === siteNorm && 
      ...
      // النقله بتاريخ اليوم تظهر في مؤكد وصول
      const todayConfirmed = records
        .filter(r => {
          const recordDate = normalizeToDateStr(r.date);
          const siteTarget = normalize(r.unloadingSite || r.customerName);
          return siteTarget === siteNorm && 
                 String(r.goodsType || '').includes(activeMaterialName) && 
                 r.status === OperationStatus.IN_PROGRESS &&
                 recordDate === todayStr;
        })
        .reduce((sum, r) => sum + Number(r.weight || 0), 0);
      ...
      const totalArrivedCumulative = records
        .filter(r => {
          const siteTarget = normalize(r.unloadingSite || r.customerName);
          return siteTarget === siteNorm && String(r.goodsType || '').includes(activeMaterialName) && r.status === OperationStatus.DONE;
        })
        .reduce((sum, r) => sum + Number(r.weight || 0), 0);
      ...
      const inTransit = records
        .filter(r => {
          const siteTarget = normalize(r.unloadingSite || r.customerName);
          return siteTarget === siteNorm && String(r.goodsType || '').includes(activeMaterialName) && r.status === OperationStatus.IN_PROGRESS;
        })
        .reduce((sum, r) => sum + Number(r.weight || 0), 0);
      ...
      const stopped = records
        .filter(r => {
          const siteTarget = normalize(r.unloadingSite || r.customerName);
          return siteTarget === siteNorm && String(r.goodsType || '').includes(activeMaterialName) && r.status === OperationStatus.STOPPED;
        })
        .reduce((sum, r) => sum + Number(r.weight || 0), 0);
      ...
      // حساب المتبقي بالميناء
      const portRemaining = totalReleased - totalArrivedCumulative - inTransit - stopped;

      //  رصيد المصنع الحالي = رصيد بداية المدة + إجمالي المنفذ امس(نقلات DONE ) - الصرف
      const currentStock = (opening + yesterdayArrived)- spending;

      if (totalReleased > 0 || opening > 0 || yesterdayArrived  > 0 || Math.abs(currentStock) > 0.001) {
        summary[key] = {
          id: manual?.id || `${siteNorm}_${activeMaterialName}`,
          site: originalName,
          siteNorm,
  ...
  }, [releases, records, factoryBalances, activeMaterialName, todayStr, yesterdayStr]);

  const displayedData = useMemo(() => {
    return data.filter(item => !hiddenSites.has(item.siteNorm));
  }, [data, hiddenSites]);
  ...
  const handleSave = async () => {
    if (!tempBalance.factoryName) return;
    setIsUpdating(true);
    try {
  ...
  const toggleHideSite = (siteNorm: string) => {
    const newSet = new Set(hiddenSites);
    if (newSet.has(siteNorm)) {
      newSet.delete(siteNorm);
    } else {
      newSet.add(siteNorm);
    }
    setHiddenSites(newSet);
  };
        ...
        <div className="flex items-center gap-3">
            {hiddenSites.size > 0 && (
              <button 
                ...
                className="bg-rose-50 text-rose-600 px-4 py-2.5 rounded-xl font-black text-xs hover:bg-rose-100 flex items-center gap-2 transition-all border border-rose-100 no-print"
              >
                <i className="fas fa-eye"></i> إظهار {hiddenSites.size} مواقع مخفية
              </button>
            )}
      ...
      <div className="bg-white rounded-[35px] shadow-2xl border border-slate-100 overflow-hidden mb-12 mx-2 print-shadow-none">
        <div className="bg-[#d97706] p-5 text-white flex justify-between items-center px-8">
           <div className="flex flex-row-reverse items-center gap-2">
              <span className="text-[10px] bg-black/10 px-2 py-0.5 rounded-md font-black no-print">{displayedData.length} مواقع</span>
          ...
          <table className="w-full text-center border-collapse min-w-[1100px]">
            <thead>
              <tr className="bg-slate-50/50 text-[11px] font-black text-slate-400 uppercase tracking-widest border-b">
                ...
                <th className="p-5 text-slate-400 font-bold border no-print">إجراء</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50 text-slate-700">
              {displayedData.map((item, idx) => {
                ...
                return (
                  <tr key={`${item.siteNorm}-${item.material}-${idx}`} className="hover:bg-slate-50/30 transition-all group">
                    ...
                    <td className="p-5 font-bold text-blue-500 border">{item.todayConfirmed.toLocaleString(undefined, {minimumFractionDigits: 3})}</td>
                    ...
                    <td className="p-5 border no-print">
                      <div className="flex gap-2 justify-center">
                        {isEditing ? (
                          <>
                            <button onClick={handleSave} className="w-8 h-8 bg-emerald-600 text-white rounded-lg flex items-center justify-center shadow-md hover:bg-emerald-700" title="حفظ"><i className="fas fa-check"></i></button>
                            <button onClick={() => setEditingKey(null)} className="w-8 h-8 bg-slate-100 text-slate-400 rounded-lg flex items-center justify-center hover:bg-slate-200" title="إلغاء"><i className="fas fa-times"></i></button>
                          </>
                        ) : (
                          <>
                            {canEdit && (
                              <button onClick={() => handleEdit(item)} className="w-8 h-8 text-indigo-600 bg-indigo-50 rounded-lg flex items-center justify-center hover:bg-indigo-600 hover:text-white transition-all shadow-sm" title="تعديل الرصيد والصرف">
                                  <i className="fas fa-edit text-xs"></i>
                              </button>
                            )}
                            <button onClick={() => toggleHideSite(item.siteNorm)} className="w-8 h-8 text-slate-400 bg-slate-50 rounded-lg flex items-center justify-center hover:bg-rose-50 hover:text-rose-600 transition-all shadow-sm" title="إخفاء من التقرير">

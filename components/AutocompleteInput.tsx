...
const AutocompleteInput: React.FC<Props> = ({ label, name, value, onChange, suggestions, placeholder, required }) => {
  ...
  const filtered = suggestions.filter(s => {
    ...
    const term = stringValue.toLowerCase();
    const itemValue = String(getValue(s)).toLowerCase();
    const itemLabel = String(getLabel(s)).toLowerCase();
    const itemSub = String(getSubLabel(s) || '').toLowerCase();
    
    return itemValue.includes(term) || itemLabel.includes(term) || itemSub.includes(term);
  }).slice(0, 30);

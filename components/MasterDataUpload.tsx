...
import { processExcelData, downloadMasterTemplate, smartReadSheet } from '../utils/excelProcessor';
...
const MasterDataUpload: React.FC<Props> = ({ 
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const rawData = smartReadSheet(ws);
        
        // Use the smart processor to extract data
        const processedUsers = processExcelData(rawData, ['name', 'pin', 'role', 'allowedMaterials']);
        const extractedUsers: AppUser[] = processedUsers.map(u => ({
          name: String(u.name || '').trim(),
          pin: String(u.pin || '').trim(),
          role: (String(u.role || 'viewer')).toLowerCase() as UserRole,
          allowedMaterials: u.allowedMaterials || 'الكل'
        })).filter(u => u.name && u.pin);

        const extractField = (key: string) => {
          return Array.from(new Set(processExcelData(rawData, [key]).map(i => String(i[key]).trim()).filter(Boolean)));
        };

        const newData: MasterData = {
          drivers: Array.from(new Set([...(draftData.drivers || []), ...extractField('driver')])),
          cars: Array.from(new Set([...(draftData.cars || []), ...extractField('car')])),
          loadingSites: Array.from(new Set([...(draftData.loadingSites || []), ...extractField('loadingSite')])),
          unloadingSites: Array.from(new Set([...(draftData.unloadingSites || []), ...extractField('unloadingSite')])),
          goodsTypes: Array.from(new Set([...(draftData.goodsTypes || []), ...extractField('goodsType')])),
          orderNumbers: Array.from(new Set([...(draftData.orderNumbers || []), ...extractField('orderNumber')])),
          contractors: Array.from(new Set([...(draftData.contractors || []), ...extractField('contractor')])),
        ...
        setHasChanges(true);
        onNotify?.(lang === 'ar' ? 'تم استيراد البيانات بذكاء (تم دمج الحقول المتشابهة)' : 'Smart data import completed (similar fields merged)', 'info');
      } catch (err) {
        console.error(err);
  ...
  const categories = useMemo(() => [
    ...
    { key: 'items' as keyof MasterData, label: t.itemsCategory || 'الأصناف', count: draftData.items?.length || 0, icon: 'fa-tag' },
  ], [draftData, t]);
          ...
          <div className="flex flex-col gap-4">
            <div className="border-2 border-dashed border-gray-100 rounded-[32px] p-10 flex flex-col items-center justify-center text-center hover:border-indigo-200 transition-all bg-gray-50/30 group">
              ...
              <label htmlFor="master-upload" className="cursor-pointer w-full">
                ...
                <p className="text-[10px] text-gray-400 mt-2">يدعم جميع أنواع التنسيقات العربية والإنجليزية بذكاء</p>
              </label>
            </div>
            <button 
              onClick={downloadMasterTemplate}
              className="flex items-center justify-center gap-3 p-4 bg-white border border-gray-200 rounded-3xl text-xs font-black text-slate-600 hover:bg-slate-50 transition-all shadow-sm"
            >
              <i className="fas fa-download text-indigo-500"></i>
              تحميل نموذج البيانات الاسترشادي (Format Template)
            </button>
          </div>
                ...
                {activeDropdown === cat.key && (
                  <div className="mt-2 bg-white border rounded-2xl shadow-2xl max-h-60 overflow-y-auto z-20 relative divide-y divide-gray-50">
                    {(draftData[cat.key] as string[] || []).map((item, idx) => (
                      <div key={`master-item-${cat.key}-${item}-${idx}`} className="flex flex-row-reverse items-center justify-between p-3 hover:bg-gray-50">

  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editRecord) {
        ...
        const autoId = `SOY-${Math.floor(1000 + Math.random() * 8999)}-${Date.now().toString().slice(-6)}`;
        ...
        <form onSubmit={handleSubmit} className="space-y-10">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-x-10 gap-y-8">
            ...
            <div className="flex flex-col gap-2">
               ...
               <select name="status" value={formData.status || OperationStatus.IN_PROGRESS} onChange={handleChange} className="w-full bg-slate-50 border-none p-5 rounded-2xl font-bold outline-none">
            ...
            <AutocompleteInput label={t.unloadingSite} name="unloadingSite" value={formData.unloadingSite || ''} onChange={handleChange} suggestions={unloadingSuggestions} placeholder={t.unloadingSite} required />
            <AutocompleteInput 
            ...
            <AutocompleteInput 
              ...
              onChange={handleChange} 
              suggestions={itemCodeSuggestions} 
            ...
            <AutocompleteInput label="رقم البوليصة" name="waybillNo" value={formData.waybillNo || ''} onChange={handleChange} suggestions={[]} placeholder="رقم البوليصة" required />
            <div className="flex flex-col gap-2">
               <label className="text-xs font-black text-slate-500 uppercase text-right">نوع المنصرف</label>
               <select 
                 name="expenditureType" 
                 value={formData.expenditureType || 'صرف عميل'} 
                 onChange={handleChange} 
                 className="w-full bg-slate-50 border-none p-5 rounded-2xl font-bold outline-none cursor-pointer"
               >
